...
```text
Input_1:เป็นคะแนนรวมของทั้งสามคน(เป็นจำนวนจริง)
Input_2:เป็นคะแนนสูงสุดของทั้งสามคน
Output:"Surprising" หรือ "Not surprising"
Constraints: ถ้าคะแนนโหวดของคนที่ให้คะแนนสูงสุด และคะแนนต่ำสุดห่างกันเกินสองคะแนนจะเป็น(Surprising)
...
```text
Step 1: ดูรูปแบบTestcases
Step 2: คำนวณโดยการใช้ max()
Step 3: ใช้if else ถ้าคะแนนสูงสุด และคะแนนต่ำสุดห่างกันเกินสองคะแนน outputจะเป็นSurprising ถ้าไม่ outputจะเป็นNot surprising
```
...
```text
วิธีสุดท้ายใช้เหมือนแผนแรก เพราะ การที่หาค่าน้อยที่สุดด้วย max(0,total_score - 2*high_score) เพราะจะทำให้ค่าต่ำสุดนี้ไม่ติดลบ ถ้าใช้//3 จะหาแค่ค่าต่ำสุดเฉยๆและถ้าค่าต่ำสุดติดลบมันจะทำให้เลขมันเพี้ยนไปทั้งหมด และจะทำให้คำตอบไม่ถูกต้อง
...
```text
ความห่างของคะแนนสูงและต่ำสุด มากกว่า2
...
```text
ความห่างของคะแนนสูงและต่ำสุด น้อยกว่า 2
...
```text
ช่วยอธิบายโค้ดที่ไม่เข้าใจ และช่วยในการคำนวณ
